...
let buildDeckWithOnlineModels
    ...
    // Build LLM prompt
    let prompt = sprintf """You are an expert Disney Lorcana deck builder. Build a competitive %d-card deck for the following request:
...
- JSON format only, no extra text""" 
        targetSize query.request format colorConstraint candidateCards targetSize
    ...
    try
        ...
        if firstBrace = -1 then
            ...
            let! cardEntries = task {
                let entries = System.Collections.Generic.List<DeckBuilder.Shared.CardEntry>()
            ...
            let explanation = sprintf """Built %d-card %s deck in %dms using online model RAG.
